...
export default async function DashboardPage() {
  // Role check is now handled in layout.tsx - no need to check here
  ...
  if (errorMessage) {
    return (
      <div>
        ...
        <Card>
          <div className="p-6">
            <h1 className="text-xl font-medium text-red-600 mb-2">Dashboard Error</h1>
  ...
  if (!brand || !summary) {
    return (
      <div>
        <PageHeader title="Dashboard" />
        <Card>
          <p className="text-gray-300">Loading brand statistics...</p>
        </Card>
      </div>
    );
  }
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        <Card>
          ...
          <p className="text-3xl font-medium text-gray-100">{summary.memberCount.toLocaleString()}</p>
        </Card>
