...
export async function adminApiFetch<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  try {
    ...
    debugError("Auth error in adminApiFetch:", errorMsg);
  ...
  if (!res.ok) {
    ...
    debugError("[adminApiFetch] Error:", errorMessage);
  ...
  // Log success with minimal preview
  const preview = Array.isArray(data)
    ? `Array(${data.length})`
    : data && typeof data === "object"
      ? `Object(${Object.keys(data).slice(0, 5).join(", ")})`
      : String(data);
  debugLog("[adminApiFetch] OK", { method, url, preview });
